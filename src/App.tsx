...
const PHASE_RULES: PhaseDefinition[] = [
  ...
  },
  {
    ...
    care: ['提前保護：早餐加蛋白質、下午一份安全點心、每餐加纖維'],
...
const PhoebeCycleTracker: React.FC = () => {
  ...
  const currentPhase = useMemo(() => {
    const phase = PHASE_RULES.find((p) => daysPassed >= p.startDay && daysPassed <= p.endDay);
  ...
  const getSymptomRecordForDate = useCallback(
    (dateStr: string): SymptomRecord | undefined => {
  ...
  const handleSaveSymptomRecord = () => {
    ...
    const isBlank =
      Object.values(currentRecord).slice(1, 5).every((v) => v === '') &&
  ...
  const handleSaveEdit = () => {
    ...
    if (updatedHistory.length >= 2) {
      const prevLength = getDaysDifference(
        updatedHistory[updatedHistory.length - 2].startDate,
        editDate
      );
          ...
          {generateCalendarDays.map((date, i) => {
            ...
            return (
              ...
              >
                ...
                {/* 顯示紀錄標記 */}
                {record && <div style={recordDotStyle} title="有個人紀錄"></div>}
              </div>
            );
          })}
        </div>
      ...
      <div style={gridContainerStyle}>
        ...
        >
          ...
          <div style={{ marginBottom: '15px' }}>
            ...
            <strong
              style={{ ...predictionDateStyle, color: PHASE_RULES[4].accent }}
            >
              {nextPMSDate}
            </strong>
          </div>
          <div>
            ...
            <strong
              style={{ ...predictionDateStyle, color: PHASE_RULES[0].accent }}
            >
              {nextPeriodDate}
            </strong>
          </div>
      ...
      {modalDetail && currentRecord && (
        <div style={modalOverlayStyle}>
          <div style={{ ...modalContentStyle, width: '400px' }}>
            ...
            <p style={modalTextStyle}>
              賀爾蒙週期: <strong>{modalDetail.phase.hormone}</strong>
            </p>
        ...
        <div style={modalOverlayStyle}>
          <div style={modalContentStyle}>
            ...
            <p style={{ marginBottom: '15px' }}>
              當前開始日：<strong>{lastStartDate}</strong>
            </p>
