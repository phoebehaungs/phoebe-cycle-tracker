...
const parseLocalDate = (dateStr: any): Date => {
  if (!dateStr || typeof dateStr !== 'string') return new Date(); // 防止崩潰
  const parts = dateStr.split('-');
  if (parts.length !== 3) return new Date(); // 防止格式錯誤
  const [y, m, d] = parts.map(Number);
  return new Date(y, m - 1, d);
};

const formatLocalDate = (date: Date): string => {
  if (!date || isNaN(date.getTime())) return "2025-01-01"; // 防止無效日期
...
const PhoebeCycleTracker: React.FC = () => {
  ...
  const [history, setHistory] = useState<CycleRecord[]>(() => {
    ...
    try {
      ...
      if (!Array.isArray(parsed) || parsed.length === 0) return INITIAL_HISTORY;
      // 確保排序與資料完整性
      return parsed
        .filter(x => x && x.startDate) // 過濾壞資料
    ...
    <div style={appContainerStyle}>
      ...
      <div style={{ ...cardStyle, marginTop: '20px', padding: '20px 15px' }}>
        ...
        <div style={{ position: 'relative', height: '140px' }}>
            ...
            <div style={{ position: 'absolute', left: `calc(${(chartDaysPassed / 34) * 100}% - 14px)`, bottom: '-22px', backgroundColor: '#555', color: 'white', fontSize: '0.65rem', padding: '2px 4px', borderRadius: '4px', fontWeight: 'bold', zIndex: 5, fontFamily: 'Noto Sans TC, sans-serif' }}>今天</div>
        </div>
